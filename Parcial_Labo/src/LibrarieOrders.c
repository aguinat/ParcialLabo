...
int CargarPedido(ePedido lista[], int tam, int reintentos, eCliente listaCli[], int tamListaCli, int* idGenerado) {
	...
	do{
		...
		if(lista[espacioLibre].isEmpty == 1 && espacioLibre != -1){
			lista[espacioLibre] = CrearPedido(&validacion, reintentos, listaCli, tamListaCli);
			if(validacion ==1){
	...
	ConfirmarContinuar("¿Desea ingresar otro producto? ", continuar);
	}while(continuar[0] == 'S');
...
int CompletarPedido(ePedido lista[], int tam, int reintentos) {
	...
	for (i = 0; i < tam; i++) {
		...
		ConfirmarContinuar("¿Desea ingresar otro producto? ", continuar);
		}while(continuar[0] == 'S' || validacionPedido == 0);
		break;
	}
...
ePedido CrearPedido(int* validacion, int reintentos, eCliente lista[], int tamClienteLista) {
	...
	int clienteExiste;
		...
		do{
			MostrarDatosCargados(lista, tamClienteLista);
